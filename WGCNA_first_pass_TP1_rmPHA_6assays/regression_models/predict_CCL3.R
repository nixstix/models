...
load('../../MOFA_4assays/data/MOFA_models/predictors.RData')

x1= data.frame(Day0.Expr.ENSG00000277632.1=predictors.baseline$Day0.Expr.ENSG00000277632.1,
               Day0.Meta.specimen_id = predictors.baseline$Day0.Meta.specimen_id,
               CCL3 = predictors.baseline$CCL3
)
               
x1$Day0.Meta.specimen_id = gsub('x', '', x1$Day0.Meta.specimen_id)
rm(predictors.baseline)

load('../../MOFA_4assays/data/MOFA_models/test.RData')

x2= data.frame(Day0.Expr.ENSG00000277632.1=test_data.baseline$Day0.Expr.ENSG00000277632.1,
               Day0.Meta.specimen_id = test_data.baseline$Day0.Meta.specimen_id,
               CCL3 = test_data.baseline$CCL3
)

x2$Day0.Meta.specimen_id = gsub('x', '', x2$Day0.Meta.specimen_id)
rm(test_data.baseline)


load('../data/MOFA_models/predictors.RData')
load('../data/MOFA_models/test.RData')

predictors.baseline = left_join(predictors.baseline, x1)

grep('specimen', colnames(test_data.baseline), value=F)
test_data.baseline = test_data.baseline[ , c(1:15, 17:ncol(test_data.baseline))]
test_data.baseline = left_join(test_data.baseline, x2)
...
features = c("Day0.Expr.ENSG00000277632.1",'Day0.Meta.age')
...
save(model1, model2, model3, models, models_alpha, file='../data/regression_models/regression_models_CCL3.RData')
