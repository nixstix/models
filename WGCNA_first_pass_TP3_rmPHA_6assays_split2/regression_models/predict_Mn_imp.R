...
features = c("Day0.Freq.Monocytes", 'Day0.Meta.age')
model1 = run_glmnet(p = features, task.name = task, alpha=1)
model1$model_coef
model1$model_cor


new_data = test_data.baseline

test_data.baseline = test_data.baseline[, c(features, task, f)]
summary(test_data.baseline)

test_data.baseline = data.frame(apply(test_data.baseline, 2, function(x){
  m = median(x, na.rm=T)
  x[is.na(x)] = m
  return(x)
...
save(model1, model2, model3, models, models_alpha, file='../data/regression_models/regression_models_Mn_imp.RData')
